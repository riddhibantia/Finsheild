...
1. **Public Tunnel**: `cloudflared tunnel --url http://127.0.0.1:8000` created a live public endpoint:
   `<YOUR-CLOUDFLARE-URL>/api/webhooks/cashfree` (example placeholder — use your own tunnel URL per run)
...
| :--- | :--- | :--- |
| **Payment Link** | `code=<EXAMPLE-PAYMENT-CODE-1>` | `code=<EXAMPLE-PAYMENT-CODE-2>` |
| **Amount** | **₹1.00** | **₹100,000.00 (₹1 Lakh)** |
| **Transaction ID** | `CF-EXAMPLE-001` | `CF-EXAMPLE-002` |
...
1. **Column 1 — Identity & Funding ($x = 115$)**: Payer Identity (`USER-EXAMPLE`) and Funding Accounts / VPAs (`user-example@example`).
...
4. **Step 4: Live Cashfree Payment Ingestion (₹1 Micro-Payment)**:
   - Locate transaction `CF-EXAMPLE-001` in the Command Center table.
...
5. **Step 5: Live Cashfree Anomaly Block (₹1,00,000 High-Value Payment)**:
   - Locate transaction `CF-EXAMPLE-002` in the Command Center table.
...
6. **Step 6: Forensic Deep Dive (5-Signal Radar)**:
   - Click "Investigate" on `CF-EXAMPLE-002` to open `/#/investigate/CF-EXAMPLE-002`.
...
10. **Step 10: Privacy & Identity Tokenization**:
    - Navigate to `/#/privacy/USER-EXAMPLE`.
