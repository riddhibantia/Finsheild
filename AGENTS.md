...
- Full-stack app (merged single repo): `backend/` (FastAPI, `backend.main:app`, auto-discovers ML root via `backend/config.py`, `FINSHEILD_CORE_PATH` override) + `frontend/` (React+Vite, `npm run dev`). Launch with `bash start_app.sh`. Test with `pytest backend/tests/ -q` (needs `PYTHONPATH` including repo root). Full guide in `docs/app.md`.
