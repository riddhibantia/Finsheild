...
def get_ml_repo_root() -> Path:
    ...
    # 2. Check sibling directory ../Finsheild (or ../Finshield — local folder spelling)
    for _sib_name in ("Finsheild", "Finshield"):
        sibling = (Path(__file__).resolve().parents[2] / _sib_name).resolve()
